...
class SwiftEditEmbedMarkTrainer(IP2PEmbedMarkTrainer):
    def _build_ip2p(self):
        dtype = torch.float16 if self.args.fp16 else torch.float32
        pipe = StableDiffusionImg2ImgPipeline.from_pretrained(
            self.args.swift_model_id,
            torch_dtype=dtype,
            safety_checker=None,
        )
        if getattr(self.args, "swift_lora_path", ""):
            pipe.load_lora_weights(self.args.swift_lora_path)
    ...
    def edit(self, img01: torch.Tensor, seed: int) -> torch.Tensor:
        ...
        outputs = self._pipe_call_with_grad(
            ...
            image=image,
            generator=g,
            strength=self.args.swift_strength,
            num_inference_steps=self.args.swift_steps,
            guidance_scale=self.args.guidance_scale,
            output_type="pt",
        )
        edited = self._normalize_pipe_output(outputs.images)
        diff = (edited - image).abs().mean(dim=1, keepdim=True)
        mask = torch.sigmoid((diff - self.args.swift_mask_threshold) * self.args.swift_mask_sharpness)
        mask = (mask * self.args.swift_blend).clamp(0.0, 1.0)
        return torch.lerp(image, edited, mask).clamp(0, 1)
