...
SHARED_ARGS = [
    "--config",              "../instructPix2Pix/configs/afhq.yml",
    "--num_user",            "100",
    "--fp16",                "1",
    "--bs_train",            "2",
    "--swift_steps",         "1",
...
def get_dog_features() -> List[str]:
    return [k for k in SRC_TRG_TXT_DIC if k.endswith("1") and k.startswith("dog_")]
...
def parse_args():
    ...
    p.add_argument("--start_from", type=str, default="dog_nicolas1")
...
def main():
    args = parse_args()
    features = get_dog_features()
    if args.start_from not in features:
        raise SystemExit(f"start_from not found in dog feature list: {args.start_from}")

    start_idx = features.index(args.start_from)
    selected = features[start_idx:]
    env = os.environ.copy()

    print(f"[batch] total dog features ending with 1: {len(features)}")
