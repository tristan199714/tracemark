...
SHARED_ARGS = [
    "--config",              "../instructPix2Pix/configs/bedroom.yml",
    "--num_user",            "100",
    "--fp16",                "1",
    "--bs_train",            "2",
    "--swift_steps",         "1",
    "--swift_strength",      "0.6",
    "--swift_mask_threshold","0.04",
    "--swift_mask_sharpness","30",
    "--swift_blend",         "1.0",
    "--embed_dim",           "128",
    "--user_dim",            "64",
    "--writer_hidden",       "96",
...
def get_bedroom_features() -> List[str]:
    return [k for k in SRC_TRG_TXT_DIC if k.endswith("1") and k.startswith("bedroom_")]
...
def parse_args():
    ...
    p.add_argument("--start_from", type=str, default="bedroom_blue1")
...
def main():
    args = parse_args()
    features = get_bedroom_features()
    if args.start_from not in features:
        raise SystemExit(f"start_from not found in bedroom feature list: {args.start_from}")

    start_idx = features.index(args.start_from)
    selected = features[start_idx:]
    env = os.environ.copy()

    print(f"[batch] total bedroom features ending with 1: {len(features)}")
