...
from train import SwiftEditEmbedMarkTrainer
...
def parse_args():
    ...
    p.add_argument("--swift_model_id", type=str, default="stabilityai/sd-turbo")
    p.add_argument("--swift_lora_path", type=str, default="")
    p.add_argument("--negative_prompt", type=str, default="")
    p.add_argument("--instruction", type=str, default="beards1")
    p.add_argument("--num_user", type=int, default=15)

    p.add_argument("--resolution", type=int, default=256)
    p.add_argument("--swift_steps", type=int, default=1)
    p.add_argument("--swift_strength", type=float, default=0.6)
    p.add_argument("--swift_mask_threshold", type=float, default=0.04)
    p.add_argument("--swift_mask_sharpness", type=float, default=30.0)
    p.add_argument("--swift_blend", type=float, default=1.0)
...
def main():
    ...
    if rank == 0:
        ...
        print(f"[SwiftEdit] log_file: {log_path}")
        print(f"[SwiftEdit] run_dir: {run_dir}")

    try:
        cfg = yaml.safe_load(open(args.config))
        if rank == 0:
            dump_run_metadata(args, cfg, run_dir)
            print("[SwiftEdit] argv:", " ".join(sys.argv))
            print("[SwiftEdit] args:", json.dumps(vars(args), ensure_ascii=False, sort_keys=True))
        trainer = SwiftEditEmbedMarkTrainer(args, cfg)
        trainer.run()
    finally:
