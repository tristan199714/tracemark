...
from train import IP2PEmbedMarkTrainer
...
def parse_args():
    ...
    p.add_argument("--config", type=str, default="configs/celeba.yml")
    p.add_argument("--exp", type=str, default=str(THIS_DIR))
    p.add_argument("--seed", type=int, default=1234)

    p.add_argument("--ip2p_model_id", type=str, default="timbrooks/instruct-pix2pix")
...
def main():
    ...
    if rank == 0:
        ...
        print(f"[EmbedMark] log_file: {log_path}")
        print(f"[EmbedMark] run_dir: {run_dir}")

    try:
        cfg = yaml.safe_load(open(args.config))
        if rank == 0:
            dump_run_metadata(args, cfg, run_dir)
            print("[EmbedMark] argv:", " ".join(sys.argv))
            print("[EmbedMark] args:", json.dumps(vars(args), ensure_ascii=False, sort_keys=True))
        trainer = IP2PEmbedMarkTrainer(args, cfg)
        trainer.run()
    finally:
